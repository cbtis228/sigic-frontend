...
@Component({
  selector: 'app-salones-management',
  imports: [
    MenuModule,
    TableModule,
    IconFieldModule,
  ...
  templateUrl: './salones-management.component.html',
  styleUrl: './salones-management.component.scss'
})
export class SalonesManagementComponent {
  constructor(
  ...
  salonData: PaginatedData<Salon> | null = null;
  ...
  ngOnInit(): void {
    this.academicoService.SalonListApi().subscribe({
      next: (response) => {
  ...
  updateMenuItems(salon: any): void {
    this.menuItems = [
      {
        label: 'General',
        icon: 'pi pi-user',
        command: () => {
          this.router.navigate([`${salon.id}/general`], {
            relativeTo: this.route,
          });
        },
      },
  ...
  onTableEvent(event?: TableLazyLoadEvent) {
    ...
    this.academicoService
      .SalonListApi(orderBy, order, filters, offset, limit)
      .subscribe({
        next: (response) => {
          this.salonData = response;
        },
